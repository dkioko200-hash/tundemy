  ...
  useEffect(() => {
    ...
    async function verify() {
      const res = await fetch(
        `/api/pesapal/verify?OrderTrackingId=${orderTrackingId}&slug=${slug}`
      );
    ...
    >
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 max-w-sm w-full text-center">
        {status === "verifying" && (
          <>
            <div
              className="w-14 h-14 rounded-full border-[3px] animate-spin mx-auto mb-6"
        ...
        {status === "failed" && (
          <>
            ...
            <button
              onClick={() => router.push(`/courses/${slug}/enroll`)}
        ...
        {status === "pending" && (
          <>
            <div
              className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-5"
              style={{ backgroundColor: "rgba(227,160,8,0.1)" }}
            >
              <svg
                width="32"
                height="32"
            ...
            <p className="text-sm text-gray-500 mb-6">
              Your payment is being processed. Check your dashboard in a moment.
            </p>
            <button
