...
export default function PreviewModal({ course, onClose, onEnroll }: PreviewModalProps) {
    ...
    >
      ...
      >
        ...
        <div className="px-6 pb-10 pt-3">
          ...
          <div className="flex items-start justify-between mb-5">
            <div className="flex items-center gap-3 min-w-0">
              <div
                ...
                style={{ backgroundColor: `${displayCourse.tag_color}18` }}
              >
                {displayCourse.icon}
              </div>
              <div className="min-w-0">
                <h2 className="text-lg font-extrabold leading-snug" style={{ color: "#0f1f3d" }}>
                  {displayCourse.title}
                </h2>
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  {displayCourse.badge_label && (
                    <span
                      className="text-xs font-bold px-2.5 py-0.5 rounded-full"
                      style={{ backgroundColor: displayCourse.badge_bg ?? "#0f1f3d", color: "#fff" }}
                    >
                      {displayCourse.badge_label}
                    </span>
                  )}
          ...
          <p className="text-sm text-gray-600 leading-relaxed mb-6">{displayCourse.description}</p>

          <div className="mb-6">
            <h3 className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#0f1f3d" }}>
              What you&apos;ll accomplish
            </h3>
            <ul className="space-y-3">
              {displayCourse.what_you_will_learn.map((item, i) => (
          ...
          >
            {[
              { label: "Lessons", value: String(displayCourse.lessons_count) },
              ...
              <div key={detail.label} className="text-center">
                <div className="text-sm font-bold leading-snug" style={{ color: "#0f1f3d" }}>
                  {detail.value}
                </div>
                <div className="text-xs text-gray-400 mt-0.5">{detail.label}</div>
              </div>
            ))}
          </div>
          ...
          >
            <div>
              <div className="text-2xl font-extrabold" style={{ color: "#0f1f3d" }}>
                KSh {displayCourse.price_kes.toLocaleString()}
              </div>
          ...
          <button
            onClick={() => { onClose(); onEnroll(displayCourse.slug); }}
            ...
            style={{ backgroundColor: "#2d8a4e", boxShadow: "0 4px 24px rgba(45,138,78,0.3)" }}
          >
            Enroll Now — KSh {displayCourse.price_kes.toLocaleString()}
          </button>
        </div>
      </div>
    </div>
